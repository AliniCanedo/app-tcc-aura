...
@section('content')
    @if (!empty($answer->id_classification == 5) && $answer->id_matricula == Auth::user()->id)
        <div class="text-center my-5">
            <h1>Você já respondeu o questionário!</h1>
            <i class="fas fa-check-circle mt-4" style="color: green; font-size: 64px"></i>
        </div>
    @else
        <form action="{{ route('coordenacao.store') }}" method="post">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <input type="hidden" name="id_classification" value="5">
            @csrf
