@extends('layouts.app')
@section('title', ($data->id > 0 ? 'Editar' : 'Novo') . ' Modelo de Resposta')

@section('content')
    <div class="row mt-4">
        <div class="col-xl-9">
            <form action="{{ route('admin.models.save', $data->id) }}" method="POST">
                @csrf
            ...
            @if ($data->id > 0)
                <form action="{{ route('admin.models.delete', $data->id) }}" method="POST" id="delete-form">
                    @csrf
                    <input type="hidden" name="id" value="{{ $data->id }}">
                </form>
            @endif
