...
@section('content')
    <form action="{{ route('client.store') }}" method="post">
        ...
        @foreach ($data as $data)
            @if ($data->modelo->id === 1 && $data->classifications->id === 2)
                ...
                <input type="textbox" id="value_id" name="value_id[]" data-flex-minlabel="Discordo"
                    data-flex-maxlabel="Concordo Totalmente" class="multiple ff-rating" data-toggle="tooltip" data-placement="bottom" title="{{$data->tooltip}}">
                <div class="row">
                    <div class="col">
        ...
        <div class="row">
            <div class="col">
                <a href="{{ route('answer.curso') }}" class="btn btn-primary">Próxima</a>
            </div>
        </div>
    </form>
@endsection
